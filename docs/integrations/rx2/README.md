...
obs
//ObservableKW(observable=io.reactivex.internal.operators.observable.ObservableFromArray@10506fa7)
```

```kotlin
val flow = Flowable.fromArray(1, 2, 3, 4, 5).k()
flow
//FlowableKW(flowable=io.reactivex.internal.operators.flowable.FlowableFromArray@f2b2b)
```

```kotlin
val subject = PublishSubject.create<Int>().k()
subject
//ObservableKW(observable=io.reactivex.subjects.PublishSubject@66251acb)
```
...
```kotlin
obs.value()
//io.reactivex.internal.operators.observable.ObservableFromArray@10506fa7
```

```kotlin
flow.value()
//io.reactivex.internal.operators.flowable.FlowableFromArray@f2b2b
```

```kotlin
subject.value()
//io.reactivex.subjects.PublishSubject@66251acb
```
